    ...
    public class Lexico : Token
    {
        ...
        public void NextToken() 
        {
            ...
            if(estado == E)
            {
                if (getContenido() [0] == '"')
                {
                    throw new Error("Error lexico: No se cerro la cadena con \"", log);
                }
                else if (getContenido() [0] == '\'')
                {
                    throw new Error("Error lexico: No se cerro el caracter con '", log);
                }
                else if (getClasificacion() == Tipos.Numero)
                {
                    throw new Error("Error lexico: Se espera un digito", log);
                }
                else
                {
                    throw new Error("Error lexico: No definido", log);
                }
            }
            else if (!FinArchivo())
            {
